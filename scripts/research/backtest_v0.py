...
# Note: this archived script lives in scripts/research/ — one extra .parent
# is required to resolve back to the project root's data/ directory.
...
def investability_mask(close: pd.DataFrame) -> pd.DataFrame:
    """Boolean DataFrame: True where the symbol has data (was listed and trading).

    Critically: this is forward-filled from the FIRST observation, not the last.
    A coin that delists would be marked uninvestable after its last observation
    (not relevant for our majors universe but worth noting for future expansion).
    """
    return close.notna()
...
def run_backtest(
    ...
    """Linear daily backtest with weekly rebalances.
...
def main() -> int:
    ...
    print("\nRunning backtest ...")
    result = run_backtest(close, target_w, p.fee_bps_per_side, lag_days=1)
    eq = result["equity"]
